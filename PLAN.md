...
- [x] **1.8 — Échéances exprimées en temps serveur** · S · `FEEL-07` · Dépend de 1.1 · 2026-09-25 · `243fc32`
  ...
  - Réalisé : `state.serverTime` (Date.now() du serveur à chaque tick), estimé par `ServerClock` ; badges, halo et durée de vie comparés à cette heure. Les champs restent des dates absolues (compacité sans enjeu depuis D6). Aucun affichage de cooldown de lancer n'existait côté client. Vérifié avec +5 min : heure estimée à 32 ms près, halo correct.

- [x] **1.3 — Visée indépendante du déplacement** · M · `GAME-04` · 2026-09-25 · `51c23cb`
  ...
  - Réalisé : la visée n'est lue qu'avec `throw` (un message sans lancer du même tick ne l'efface pas) et consommée avec lui. Clavier + souris : le curseur vise dès que la souris bouge, et le clic droit ne fait plus repasser le déplacement en mode souris. Mobile : le lancer part au relâcher de THROW (tap = direction de marche, glisser = visée, glisser ramené au centre = annulé). Indicateur au sol dans `client/src/entities/AimIndicator.ts`, avec cooldown prédit à l'envoi, et bouton THROW grisé pendant le cooldown. Vérifié dans Chromium : lancer derrière soi en fuyant (Espace et clic droit), glisser en diagonale suivi à 0,1° près à l'écran, tap et annulation. Bots : ils visent la cible poursuivie, le poursuivant (Hunter, Aggressive) ou la caisse récoltée. Avec une visée précise (0,08 à 0,2 rad), les éliminations par lancer doublaient ; la dispersion est calibrée avec `tools/bench-survival.js` (nouveau banc, base de celui de 3.2) : sur 16 graines, les morts par lancer d'un débutant restent au même niveau (19 avant, 18 après), les bots s'éliminent davantage entre eux (+55 %, surtout en ripostant quand ils fuient).

- [x] **1.4 — Direction souris par projection sur le sol** · S · `FEEL-03` · À faire avec 1.3 · 2026-09-25 · `51c23cb`
  ...
  - Réalisé : projection par une caméra sans screen shake (`CameraRig.groundAt`), depuis la position dessinée du joueur ; zone morte de 40 px autour du joueur à l'écran. Position du curseur suivie sur toute la fenêtre (au-dessus du HUD, la direction restait figée). Mesure sur 7 cibles : écart ≤ 0,32° (arrondi au pixel du curseur), contre 7,3° au pire avec l'ancien calcul.

- [x] **1.5 — Retours de combat immédiats** · M · `FEEL-05` · 2026-09-25 · `d5a4524`
  ...
  - Réalisé : flash par attribut d'instance (`aFlash`, mélange vers le blanc en fin de shader). Six sons de timbres différents (tintement métallique, éclat de bruit aigu, deux tons descendants en dent de scie, carillon FM montant, craquement de bruit grave, coup sourd), conçus pour se distinguer mais pas encore validés à l'écoute : à confirmer en playtest. Repères et « +1 » en DOM (`client/src/ui/CombatFeedback.ts`), visibles à toutes les qualités. Le repère vise l'attaquant (`byId` ajouté à `BladeDestroyedEvent`) : le point de rupture seul s'en écartait jusqu'à 100° quand les orbites se chevauchent ; mesuré à 0,3° près dans un duel scripté à deux clients. Au passage, bug corrigé : `ClashEvent.aId/bId` sont des ids de lame et le client les comparait à l'id du joueur, donc un clash du joueur local était traité comme lointain (son atténué, pas de secousse) ; l'évènement porte désormais `aOwnerId/bOwnerId`, et la secousse rétablie est bornée (une par 200 ms, trauma ≤ 0,5) en attendant le lissage de 1.7. Coups sur les caisses joués sur la ligne de temps du rendu. Le fil des éliminations reste dans 3.4. Complément (`07e166c`) : entre raretés égales, les deux lames cassent au clash et disparaissaient avant la fin du flash, qui ne se voyait donc presque jamais ; la lame brisée reste en blanc jusqu'à la fin du flash.

- [x] **1.6 — Hitlag sans blocage** · S · `GAME-07` · À valider en playtest · 2026-09-25 · `1d8aac6`
  ...
  - Réalisé : 250 ms de liberté garantie après chaque gel (plutôt qu'un gel par 250 ms depuis son début, qui laissait 110 / 250 = 44 % au tier 2), et hitlag du tier 2 ramené de 110 à 100 ms : 29 % figé au pire. Orbites jamais gelées. Recul conservé pendant le gel ; il a fallu le plafonner (`KNOCKBACK_MAX_SPEED`, recul d'un clash de tier 2) : il s'additionne à chaque paire de lames au contact, et deux joueurs de tier 2 en empilaient déjà 200 u/s avant la tâche (projection à plus de 10 u), 30 u une fois conservé. Duel à 30 lames où les deux joueurs se poussent : le recul les séparait déjà, 23 % figés avant, 10 % après. Cas bloquant, joueur pris en étau entre deux adversaires pendant 3 s (8 graines) : 40 % figé avant (jusqu'à 50 %), 24 % après (27 % au plus), sortie de l'étau en moins de 0,6 s ; figé en test d'intégration. Banc de survie des débutants (8 graines) : 37 % de vies perdues avant 30 s avant, 25 % après. Ressenti (poids des impacts, recul) à valider en playtest.

- [x] **1.7 — Caméra équitable et dynamique** · M · `FEEL-04` · 2026-09-25 · `b68479b`
  ...
  - Réalisé : cadrage de Néon (55°, 54°, 27 u) pour tous. Mesuré dans Chromium au niveau du joueur : 49,93 u de largeur sur les quatre thèmes en 1280×720 ; 40,9 u en 390×844, soit 82 % (26 % avant) ; 4:3 compensé à 82 % ; paysage mobile 844×390 à 122 %, non plafonné (seul un minimum est garanti). Recul avec l'orbite : 27 → 28,3 u au deuxième anneau (18 lames), par pas proportionnels au temps de frame. La compensation se fait en reculant la caméra, pas en élargissant le champ (au-delà de 70°, la déformation devient forte) : en portrait, la caméra est à 85 u, un joueur fait une dizaine de pixels CSS de large, et on voit plus loin devant soi qu'en 16:9 (le brouillard, décalé avec la distance, en masque le fond). Taille à juger en playtest mobile ; levier : `CAMERA_MIN_VIEW_WIDTH`. Visée (1.4) revérifiée avec la nouvelle caméra.

- [x] **1.2 — Vraie prédiction avec rejeu d'inputs** · L · `FEEL-02` · Dépend de 1.1 et T.2 · 2026-09-25 · `575b939`
  ...
  - Réalisé : `stepMovement` (`shared/src/movement.ts`) et `InputPredictor` (`shared/src/prediction.ts`, la logique de rejeu du client, testable en Node). Le serveur applique un pas par input reçu, dans l'ordre, avec un crédit d'un pas par `SERVER_DT` écoulé plafonné à 10 (pas de speed-hack, rattrapage d'un à-coup réseau) ; sans input, pas de pas, ce qui rend inutile la règle des 500 ms de 0.5. Le client envoie un input par `SERVER_DT` de temps réel quel que soit le framerate (avant : au plus un par frame, soit 48 par seconde à 144 Hz). Recul synchronisé pour être rejoué. Piège trouvé en route : les inputs envoyés avant le premier état reçu étaient ignorés par la prédiction et appliqués par le serveur, d'où une correction unique de 5 pas (0,92 u) à l'entrée en jeu. Mesures dans Chromium avec 150 ms d'aller-retour (latence injectée autour du WebSocket) : correction nulle en ligne droite, aux virages, en zigzag et à l'arrêt ; demi-tour pris à la frame suivante sans aucun retour en arrière ; arrêt net. Test Node à 167 ms d'aller-retour (`server/test/prediction.test.ts`) : correction ≤ 1,5e-6 u (arrondi float32) hors évènements serveur ; Speed, recul et hitlag ne coûtent qu'une correction, quand le client les apprend. Speed non croisé pendant le test navigateur, couvert par le test Node. Banc serveur : tick moyen 1,64 ms, p99 4,0 ms.
